defmodule MarshalTest do
  ...
  test "Decode True" do
    assert Marshal.decode("\x04\bT") == true
  end

  test "Decode False" do
    assert Marshal.decode("\x04\bF") == false
  end

  test "Decode 0" do
    assert Marshal.decode("\x04\bi\x00") == 0
  end

  test "Decode 1" do
    assert Marshal.decode("\x04\bi\x06") == 1
  end

  test "Decode -1" do
    assert Marshal.decode("\x04\bi\xFA") == -1
  end

  test "Decode 500" do
    assert Marshal.decode("\x04\bi\x02\xF4\x01") == 500
  end

  test "Decode -500" do
    assert Marshal.decode("\x04\bi\xFE\f\xFE") == -500
  end

  test "Decode max fixnum" do
    assert Marshal.decode("\x04\bi\x04\xFF\xFF\xFF?") == 1073741823
  end

  test "Decode min fixnum" do
    assert Marshal.decode("\x04\bi\xFC\x00\x00\x00\xC0") == -1073741824
  end

  test "Decode empty array" do
    assert Marshal.decode("\x04\b[\x00") == []
  end

  test "Array of ints" do
    assert Marshal.decode("\x04\b[\bi\x06i\ai\b") == [1, 2, 3]
  end

  test "Mixed array" do
    assert Marshal.decode("\x04\b[\f0TFi\x06i\ai\bi\x04\xFF\xFF\xFF?") == [nil, true, false, 1, 2, 3, 1073741823]
  end

  test "Nested array" do
    assert Marshal.decode("\x04\b[\b0[\aTF[\bi\x06i\ai\b") == [nil, [true, false], [1, 2, 3,]]
  end

  test "Hash" do
    assert Marshal.decode("\x04\b{\x06i\x06i\a") == %{1 => 2}
  end
  ...
  test "Decode symbol array" do
    assert Marshal.decode("\x04\b[\a:\napple:\vbanana") == [:apple, :banana]
  end

  test "Decode repeated symbol array" do
    assert Marshal.decode("\x04\b[\b:\napple:\vbanana;\x00") == [:apple, :banana, :apple]
  end

  test "Decode repeated symbol array with nesting" do
    assert Marshal.decode("\x04\b[\n:\napple:\vbanana;\x00[\a:\bcar;\x06;\a") == [:apple, :banana, :apple, [:car, :banana], :car]
  end

  test "Decode string with UTF-8 set" do
    assert Marshal.decode("\x04\bI\"\napple\x06:\x06ET") == {"apple", [E: true]}
  end

  test "Decode string with US-ASCII set" do
    assert Marshal.decode("\x04\bI\"\nhello\x06:\x06EF") == {"hello", [E: false]}
  end

  test "Decode string with Shift_JIS set" do
    assert Marshal.decode("\x04\bI\"\nhello\x06:\rencoding\"\x0EShift_JIS") == {"hello", [encoding: "Shift_JIS"]}
  end

  test "Repeated string" do
    hello = {"hello", [E: true]}
    assert Marshal.decode("\x04\b[\aI\"\nhello\x06:\x06ET@\x06") == [hello, hello]
  end

  test "Arrays cache before internals" do
    a = {"a", [E: true]}
    assert Marshal.decode("\x04\b[\b[\x06I\"\x06a\x06:\x06ET[\x06@\a[\a@\a@\a") == [[a], [a], [a, a]]
  end

  test "Hashes cache like arrays" do
    a = %{1 => 2}
    assert Marshal.decode("\x04\b[\b[\x06{\x06i\x06i\a[\x06@\a[\a@\a@\a") == [[a], [a], [a, a]]
  end

  test "Translated string" do
    assert Marshal.decode("\x04\bI\"\x06\xC5\x06:\rencoding\"\x0EShift_JIS") == {<<0xC5>>, [encoding: "Shift_JIS"]}
  end

  test "Decode string class" do
    assert Marshal.decode("\x04\bc\vString") == {:class, "String"}
  end

  test "Decode Math::DomainError class" do
    assert Marshal.decode("\x04\bc\x16Math::DomainError") == {:class, "Math::DomainError"}
  end

  test "Cached class" do
    class = {:class, "String"}
    object = %{1 => 2}
    assert Marshal.decode("\x04\b[\tc\vString{\x06i\x06i\a@\a@\x06") == [class, object, object, class]
  end

  test "Decode module Enumerable" do
    assert Marshal.decode("\x04\bm\x0FEnumerable") == {:module, "Enumerable"}
  end

  test "Cached module" do
    module = {:module, "Enumerable"}
    object = %{1 => 2}
    assert Marshal.decode("\x04\b[\tm\x0FEnumerable{\x06i\x06i\a@\a@\x06") == [module, object, object, module]
  end

  test "Decode user object instance" do
    object_instance = {:object_instance, :DumpTest, ["@a": nil]}
    assert Marshal.decode("\x04\bo:\rDumpTest\x06:\a@a0") == object_instance
  end

  test "Cache user object instance" do
    object_instance = {:object_instance, :DumpTest, ["@a": nil]}
    assert Marshal.decode("\x04\b[\ao:\rDumpTest\x06:\a@a0@\x06") == [object_instance, object_instance]
  end

  test "Decode float" do
    assert Marshal.decode("\x04\bf\x0E12.256611") == 12.256611
  end

  test "Cache float" do
    assert Marshal.decode("\x04\b[\af\n12.11@\x06") == [12.11, 12.11]
  end
  ...
  test "Decode U type" do
    ...
    assert Marshal.decode(date_marshal) == date
  end
  ...
  test "Struct with cache" do
    ...
    assert Marshal.decode(marshal) == [struct, foo, bar, struct]
  end
  ...
  test "Extended object" do
    ...
    assert Marshal.decode(marshal) == extended
  end
  ...
  test "Nested extensions" do
    ...
    assert Marshal.decode(marshal) == overextended
  end
  ...
  test "Hash with default" do
    ...
    assert Marshal.decode(marshal) == hash_def
  end
  ...
  test "Hash def" do
    ...
    assert Marshal.decode(marshal) == defined_class
  end
  ...
  test "Float infinity" do
    ...
    assert Marshal.decode(marshal) == infinity
  end
  ...
  test "Float negative infinity" do
    ...
    assert Marshal.decode(marshal) == neg_infinity
  end
  ...
  test "Float not a number" do
    ...
    assert Marshal.decode(marshal) == nan
  end
  ...
  test "Recursive cache" do
    ...
    assert Marshal.decode(marshal) == recursive_string
  end
  ...
  test "Standard ivar order" do
    ...
    assert Marshal.decode(marshal) == [string_with_ivar, string_with_ivar, string]
  end
  ...
  test "ivar order is different for user defined object" do
    ...
    assert Marshal.decode(marshal) == [time_with_ivar, apple, tz, time_with_ivar]
  end
  ...
  test "extended object cache" do
    ...
    assert Marshal.decode(marshal) == [object, banana, object]
  end
  ...
  test "usr class nested cache" do
    ...
    assert Marshal.decode(marshal) == [usrclass, banana, cherry, usrclass]
  end
  ...
  test "hashdef nested cache" do
    ...
    assert Marshal.decode(marshal) == [hashdef, banana, cherry, hashdef]
  end
  ...
  test "struct nested cache" do
    ...
    assert Marshal.decode(marshal) == [struct, banana, cherry, struct]
  end
  ...
  test "defined struct nest cache" do
    ...
    assert Marshal.decode(marshal) == [def_struct, banana, cherry, grape, def_struct]
  end
end
